...
export default function RequestHousingPage() {
  ...
  const [furnished, setFurnished] = useState(FURNISHED_OPTIONS[3]);
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!rentValue.trim()) {
      setMessage("请填写数字预算。预算只能输入数字，例如 1200 或 1200.50。");
      return;
    }

    if (!isValidMoneyInput(rentValue)) {
      setMessage("预算只能输入数字，最多保留两位小数。例如 1200 或 1200.50。");
      return;
    }
    ...
    if (manualTags.length === 0) {
      setMessage("请至少添加一个手动标签，方便别人搜索到你的求租需求。");
      return;
    }
    ...
    <main className="min-h-screen bg-neutral-950 text-white">
      <section className="mx-auto max-w-5xl px-6 py-8">
        ...
        <section className="mt-10">
          <p className="text-sm text-neutral-400">发布求租需求</p>

          <h1 className="mt-3 text-4xl font-bold">想找一套房子</h1>

          <p className="mt-4 max-w-2xl text-sm leading-6 text-neutral-400">
            发布时会保存你当前公开的联系方式。之后即使你在个人主页关闭联系方式，这个求租帖仍会保留发布时的联系信息。
          </p>
        </section>
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-6">
          <section className="rounded-3xl border border-neutral-800 bg-neutral-900/40 p-6">
            ...
            <div className="mt-6 grid gap-5 md:grid-cols-2">
              <div className="md:col-span-2">
                <label className="text-sm text-neutral-400">求租标题</label>

                <input
                  value={title}
                  onChange={(event) => setTitle(event.target.value)}
                  placeholder="例如：求 CMU 附近 1B1B / 求 Pitt 暑期短租"
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="例如：以内 / 可议 / 包水网优先"
                  className="mt-2 w-full rounded-2xl border border-neutral-800 bg-neutral-950 px-4 py-3 text-white outline-none placeholder:text-neutral-600 focus:border-neutral-500"
                />
              </div>

              <div>
                <label className="text-sm text-neutral-400">期望房型</label>
              ...
              <div>
                <label className="text-sm text-neutral-400">家具偏好</label>
              ...
              <div>
                <label className="text-sm text-neutral-400">水电网偏好</label>
              ...
              <div>
                <label className="text-sm text-neutral-400">室友偏好</label>
              ...
              <div className="md:col-span-2">
                <SchoolSearchSelect
                  ...
                  helperText="选择这个求租需求主要面向的学校，例如 CMU、PITT、WFU、SAIC。"
              ...
              <div className="md:col-span-2">
                <label className="text-sm text-neutral-400">期望位置</label>

                <input
                  value={location}
                  onChange={(event) => setLocation(event.target.value)}
                  placeholder="例如：CMU 附近 / Oakland / WFU 附近"
                  className="mt-2 w-full rounded-2xl border border-neutral-800 bg-neutral-950 px-4 py-3 text-white outline-none placeholder:text-neutral-600 focus:border-neutral-500"
                />
              </div>

              <div>
                <label className="text-sm text-neutral-400">入住开始日期</label>
          ...
          <section className="rounded-3xl border border-neutral-800 bg-neutral-900/40 p-6">
            <h2 className="text-xl font-semibold">标签和描述</h2>
            ...
            <div className="mt-6">
              <TagInput
                ...
                label="手动标签"
                placeholder="输入标签后按 Enter，例如 暑期、近CMU、1B1B"
                helperText="标签用于搜索。描述是给人看的，标签是给系统匹配用的。"
              />
            </div>

            <div className="mt-6">
              <label className="text-sm text-neutral-400">求租描述</label>

              <textarea
                value={description}
                onChange={(event) => setDescription(event.target.value)}
                placeholder="写清楚预算、期望位置、租期、房型、是否需要家具、室友偏好等。"
                rows={7}
