...
function formatPriceLabel(priceValue: string, priceNote: string) {
  const base = `$${priceValue.trim()}`;
...
export default function SellPage() {
  ...
  const [itemCondition, setItemCondition] = useState(CONDITION_OPTIONS[1]);
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!priceValue.trim()) {
      setMessage("请填写数字价格。价格只能输入数字，例如 80 或 80.50。");
      return;
    }

    if (!isValidMoneyInput(priceValue)) {
      setMessage("价格只能输入数字，最多保留两位小数。例如 80 或 80.50。");
      return;
    }
    ...
    if (manualTags.length === 0) {
      setMessage("请至少添加一个手动标签，方便别人搜索到这个商品。");
      return;
    }
    ...
    <main className="min-h-screen bg-neutral-950 text-white">
      <section className="mx-auto max-w-5xl px-6 py-8">
        ...
        <section className="mt-10">
          <p className="text-sm text-neutral-400">发布二手商品</p>
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-6">
          <section className="rounded-3xl border border-neutral-800 bg-neutral-900/40 p-6">
            ...
            <div className="mt-6 grid gap-5 md:grid-cols-2">
              <div className="md:col-span-2">
                <label className="text-sm text-neutral-400">商品标题</label>

                <input
                  value={title}
                  onChange={(event) => setTitle(event.target.value)}
                  placeholder="例如：27寸 4K 显示器 / 宜家书桌 / 空气炸锅"
              ...
              <div>
                <label className="text-sm text-neutral-400">价格数字</label>
              ...
              <div>
                <label className="text-sm text-neutral-400">价格补充说明</label>

                <input
                  value={priceNote}
                  onChange={(event) => setPriceNote(event.target.value)}
                  placeholder="例如：可小刀 / 原价 150 / 免费赠送可填 0"
                  className="mt-2 w-full rounded-2xl border border-neutral-800 bg-neutral-950 px-4 py-3 text-white outline-none placeholder:text-neutral-600 focus:border-neutral-500"
                />
              </div>
              ...
              <div>
                <label className="text-sm text-neutral-400">成色</label>
              ...
              <div>
                <label className="text-sm text-neutral-400">取货方式</label>
              ...
              <div>
                ...
                <div className="mt-2 rounded-2xl border border-neutral-800 bg-neutral-950 px-4 py-3 text-neutral-300">
                  {TRADE_MODE_TEXT}
                </div>
              </div>
              ...
              <div className="md:col-span-2">
                <SchoolSearchSelect
                  ...
                  helperText="选择这个商品主要面向的学校，例如 WFU、CMU、PITT、SAIC。"
              ...
              <div className="md:col-span-2">
                ...
                <input
                  ...
                  placeholder="例如：CMU 附近 / WFU 校园附近 / Downtown Pittsburgh"
                  className="mt-2 w-full rounded-2xl border border-neutral-800 bg-neutral-950 px-4 py-3 text-white outline-none placeholder:text-neutral-600 focus:border-neutral-500"
                />
              </div>
            </div>
          ...
          <section className="rounded-3xl border border-neutral-800 bg-neutral-900/40 p-6">
            ...
            <div className="mt-6">
              <label className="text-sm text-neutral-400">商品图片</label>
            ...
            <div className="mt-6">
              <label className="text-sm text-neutral-400">商品描述</label>

              <textarea
                value={description}
                onChange={(event) => setDescription(event.target.value)}
                placeholder="写清楚尺寸、购买时间、使用情况、瑕疵、是否可议价、取货方式等。"
                rows={7}
